...
export default async function handler (req, res) {
  ...
  if (!dataLogin.user || !dataLogin.password) {
    return (
      res.json({ res: 'invalid username or password' }).status(401)
    )
  }
  const reqData = await mySqlDbConnect({ query: 'SELECT password FROM users WHERE `name` = ?', params: [dataLogin.user] })
